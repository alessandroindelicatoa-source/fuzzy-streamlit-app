...
def check_password():
    ...
    if not users:
        st.error(
            ...
            "If you're on Streamlit Cloud, open **Manage app → Settings → Secrets** and add:\n\n"
            "[passwords]\nuser = \"yourpassword\"\n\n"
            "If local, create **.streamlit/secrets.toml** with the same content."
        )
        st.stop()
    ...
    def _enter():
        ...
        if ok and "password" in st.session_state:
            del st.session_state["password"]  # do not keep password in memory
        ...
        for j in range(n):
            ...
            if is_benefit[j]:
                ...
                row.append(
                    TFN(
                        amin / x.c,
                        amin / (x.b if x.b != 0 else 1e-9),
                        amin / (x.a if x.a != 0 else 1e-9),
                    )
                )
        out.append(row)
    return out
...
def fuzzy_topsis_cc(matrix: List[List[TFN]], is_benefit: List[bool], weights: Optional[List[float]] = None) -> np.ndarray:
    """Return closeness coefficients ∈ [0,1] for each row."""
    m = len(matrix)
    n = len(matrix[0])
    if weights is None:
        weights = [1.0 / n] * n
...
def df_to_tfn_matrix_per_item(
    ...
    """Build matrix when EACH item has its own TFN mapping."""
    m = df.shape[0]
    mat: List[List[TFN]] = []
    for i in range(m):
        row: List[TFN] = []
...
# ==============================
# 3) ECO-Extended 4×4 sets & labels
# ==============================
def eco_fuzzy_sets_4(val: float) -> Tuple[float, float, float, float]:
    """Memberships to (Low, MedLow, MedHigh, High) over [0,1]."""
    low = medlow = medhigh = high = 0.0
    if val <= 0.33:
        low = 1 - val / 0.33
    if val >= 0.66:
        high = (val - 0.66) / 0.34 if val <= 1 else 1
    if 0 <= val <= 0.66:
        medlow = 1 - abs(val - 0.33) / 0.33
    if 0.33 <= val <= 1:
        medhigh = 1 - abs(val - 0.66) / 0.34
    return (max(low, 0), max(medlow, 0), max(medhigh, 0), max(high, 0))


def eco_extended_labels_4x4(
    x: np.ndarray,
    y: np.ndarray,
    x_names=("LowX", "MedLowX", "MedHighX", "HighX"),
    y_names=("LowY", "MedLowY", "MedHighY", "HighY"),
) -> List[str]:
    labels = []
...
# ==============================
# 4) Classic Apostle (2×2)
# ==============================
def apostle_quadrants(
    x: np.ndarray, y: np.ndarray, x_thr: float, y_thr: float, q_AA: str, q_AB: str, q_BA: str, q_BB: str
) -> List[str]:
    out = []
    for xi, yi in zip(x, y):
...
# 5) UI
# ==============================
st.set_page_config(page_title="Fuzzy Quadrant Mapper: Hybrid TOPSIS + Apostle", layout="wide")

st.title("Fuzzy Quadrant Mapper: Classic & ECO-Extended Apostle with Hybrid TOPSIS")
st.markdown(
    """
**Indelicato & Martín (2025)** — Interactive app to:
- Build two **latent variables** from items.
- Convert **per-item scales** into **Triangular Fuzzy Numbers (TFNs)**.
- Compute **Fuzzy Hybrid TOPSIS**.
- Classify into **Classic (2×2)** and **ECO-Extended (4×4)** quadrants.
"""
)
...
# ---- Step 1: choose items for two latents ----
st.header("1) Select items for the TWO latent variables")
...
if df is not None and len(df.columns) > 0:
    ...
    def _bcw(side_name: str, items: List[str], key_prefix: str):
        ...
        for c in items:
            ...
            with c1:
                bc_list.append(
                    st.selectbox(f"{side_name} — {c}: Benefit/Cost", ["Benefit", "Cost"], key=f"bc_{key_prefix}_{c}")
                    == "Benefit"
                )
            with c2:
                w_list.append(float(st.number_input(f"{side_name} — {c}: Weight", value=1.0, step=0.1, key=f"w_{key_prefix}_{c}")))
    ...
    selected_items_all = list(dict.fromkeys((items_x or []) + (items_y or [])))  # unique


# ---- Step 2: per-item TFN conversion ----
st.header("2) Define per-item TFN conversion (manual, point-by-point)")
st.caption("For each selected item, list its discrete scale (e.g., 1–10) and a TFN (a,b,c) in [0,100] for each point.")
    ...
    for it in selected_items_all:
        with st.expander(f"TFN mapping for item: {it}", expanded=False):
            ...
            try:
                ...
                st.warning(f"Invalid levels for {it}. Use integers like 1,2,3,...")
...
if use_custom_16:
    for iy in range(4):
        ...
        for ix in range(4):
            ...
            custom16[(ix, iy)] = rcols[ix].text_input(f"Cell X{ix+1}-Y{iy+1}", value=default_name, key=f"cell_{ix}_{iy}")

# ---- Step 4: thresholds & aggregation ----
...
if df is not None:
    agg_cols_indep = st.multiselect(
        "Aggregate Fuzzy-Hybrid TOPSIS independently by these categorical columns",
...
if run_btn:
    try:
        if df is None:
            ...
            st.stop()
        if len(latents) != 2:
            st.error("Please choose items for TWO latents (X and Y).")
            st.stop()
        for it in selected_items_all:
            if it not in tfn_map_by_item or it not in levels_by_item or not levels_by_item[it]:
                st.error(f"Missing TFN mapping for item '{it}'. Open its panel and define (a,b,c) for each point.")
                st.stop()
        ...
        ax1.axvline(x_thr, linestyle="--")
        ax1.axhline(y_thr, linestyle="--")
        ax1.set_xlabel(f"TOPSIS index ({xnm})")
        ax1.set_ylabel(f"TOPSIS index ({ynm})")
        ax1.set_title("Classic 2×2")
        st.pyplot(fig1)
        buf1 = BytesIO()
        fig1.savefig(buf1, format="png", dpi=300, bbox_inches="tight")
        st.download_button("⬇ Download 2×2 plot (PNG)", data=buf1.getvalue(), file_name="apostle_classic_2x2.png", mime="image/png")
        plt.close(fig1)
        ...
        ax2.set_xlabel(f"TOPSIS index ({xnm})")
        ax2.set_ylabel(f"TOPSIS index ({ynm})")
        ax2.set_title("ECO-Extended 4×4")
        st.pyplot(fig2)
        buf2 = BytesIO()
        fig2.savefig(buf2, format="png", dpi=300, bbox_inches="tight")
        st.download_button("⬇ Download 4×4 plot (PNG)", data=buf2.getvalue(), file_name="eco_extended_4x4.png", mime="image/png")
        plt.close(fig2)
        ...
        if agg_cols_indep:
            for gcol in agg_cols_indep:
                ...
                for cat, subdf in df.groupby(gcol, dropna=False):
                    ...
                    gx_items = latents[xnm]
                    gy_items = latents[ynm]
                    ...
                    z_x = fuzzy_topsis_cc(
                        df_to_tfn_matrix_per_item(subdf, gx_items, tfn_map_by_item, levels_by_item),
                        is_benefit=bene_x,
                        weights=w_x,
                    )
                    z_y = fuzzy_topsis_cc(
                        df_to_tfn_matrix_per_item(subdf, gy_items, tfn_map_by_item, levels_by_item),
                        is_benefit=bene_y,
                        weights=w_y,
                    )
                    ...
                    # classic label (global thresholds)
                    ...
                    lx = np.array(eco_fuzzy_sets_4(gx))
                    ly = np.array(eco_fuzzy_sets_4(gy))
                    ix = int(lx.argmax())
                    iy = int(ly.argmax())
                    row["Extended4x4_Label"] = custom16.get((ix, iy), f"{x_names[ix]}|{y_names[iy]}") if use_custom_16 else f"{x_names[ix]}|{y_names[iy]}"
                    rows.append(row)

                agg_one = pd.DataFrame(rows)
                st.dataframe(agg_one, use_container_width=True)
                st.download_button(
                    f"⬇ Download aggregated by {gcol} (CSV)",
        ...
        tbl_classic = pd.DataFrame(
            {"count": cnt_classic, "percent": (cnt_classic / max(1, cnt_classic.sum()) * 100).round(2)}
        )
        st.subheader("Classic (2×2)")
        st.dataframe(tbl_classic, use_container_width=True)
        st.download_button(
            "⬇ Download Classic counts (CSV)",
